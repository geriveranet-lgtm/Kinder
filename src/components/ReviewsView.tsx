...
export const ReviewsView = ({ reviews, movies, books, onAddContent, onContentClick }: any) => {
  ...
  return (
    <div className="h-full overflow-y-auto p-4 pb-32 space-y-6">
            ...
            <div key={review.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold flex items-center gap-1 ${MovieService.getTitleColor(userTitle)}`}>
                  ...
                  <p className="text-sm text-gray-500">
                    Рецензия на:{' '}
                    <span 
                      className={`font-semibold ${content ? 'text-indigo-600 cursor-pointer hover:underline' : 'text-gray-800'}`}
                ...
                {content?.posterUrl && (
                  <img 
                    ...
                    alt="" 
                    className="w-10 h-14 object-cover rounded ml-auto cursor-pointer hover:opacity-80 transition-opacity" 
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="ml-auto flex items-center gap-4">
                  ...
                  <button onClick={() => handleLike(review)} className="flex items-center gap-1 text-sm text-gray-500 hover:text-red-500 transition-colors">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-[100]" onClick={() => setIsModalOpen(false)}>
          <div className="bg-white p-6 rounded-2xl w-full max-w-md max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">Найти фильм или книгу</h2>
            ...
            <div className="overflow-y-auto flex-1 space-y-2 pr-2">
              {contentSearchTerm.trim() === '' ? (
                <p className="text-center text-gray-500 py-4">Введите название для поиска</p>
              ) : (
                [...movies, ...books]
                  ...
                  .map(content => (
                    ...
                    >
                      {content.posterUrl ? (
                        <img src={content.posterUrl} alt="" className="w-12 h-16 object-cover rounded-lg" referrerPolicy="no-referrer" />
                      ) : (
                        <div className="w-12 h-16 bg-gray-200 rounded-lg flex items-center justify-center">
                          {content.type === 'movie' ? <Film className="w-6 h-6 text-gray-400" /> : <BookOpen className="w-6 h-6 text-gray-400" />}
                        </div>
                      )}
                      <div>
                        <p className="font-bold text-gray-900">{content.title}</p>
                        <p className="text-sm text-gray-500">{content.releaseYear} • {content.type === 'movie' ? 'Фильм' : 'Книга'}</p>
                      </div>
                    </div>
                  ))
              )}
